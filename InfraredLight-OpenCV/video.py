    ...
    while True:
      ...
      while True:
        ...
        for c in contours:
          ...
          logger.add("video_info.log", rotation = "500MB", enqueue = True, compression='zip')
          logger.info("time = " + time.strftime("%Y-%m-%d-%H_%M_%S", time.localtime(time.time())) + ", alert_x = {alert_x}, alert_y = {alert_y}, alert_w = {alert_w}, alert_h = {alert_h}" \
